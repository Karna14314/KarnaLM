...
OUT_DIR      = Path("/mnt/scratch/shards")
...
# Use specific worker count per user request
NUM_WORKERS  = 20
log.info(f"Using {NUM_WORKERS} workers for tokenization.")
...
def detect_language_from_path(filepath: Path) -> str:
    ...
    # Check for keywords or directory names
    if "english" in s or "fineweb" in s or "/en/" in s or "_en" in s:
        return "english"
    elif "hindi" in s or "cc100_hi" in s or "/hi/" in s or "_hi" in s:
        return "hindi"
    elif "telugu" in s or "cc100_te" in s or "/te/" in s or "_te" in s:
        return "telugu"
    return "unknown"
...
TARGET_TOKENS = {
    "english": 10_000_000_000,  # 10B tokens (50%)
    "hindi":   7_000_000_000,   # 7B tokens (35%)
    "telugu":  5_000_000_000,   # 5B tokens (Set high to guarantee 100% consumption)
}

# Global safety cap (20B tokens = ~40GB binary)
TOTAL_TOKEN_CAP = 21_000_000_000
...
def tokenize_parquet_file(args):
    ...
    try:
        ...
        # Use batch_size=1000 per user request to manage memory
